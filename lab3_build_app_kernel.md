...
----------------------------------------------------------------------------
```
The input, out and reference software output are shown below for filter type selection set to 3:
...
## System Run
In this section we will run the host application using FPGA hardware and analyze the performance of overall system using Vitis Analyzer and host application console output.
...
#### Kernel Latency and Bandwidth Utilization 
 While application is run using actual FPGA hardware or in emulation mode a run time trace can be generated. This trace can be viewed within Vitis Analyzer for details please refer to other [Vitis Tutorials](https://github.com/Xilinx/Vitis-Tutorials) that discuss such details or Vitis user guide. The trace information generated during application run can be controlled by different options inside a file that is always named as "xrt.ini" and placed in the same folder as host application. You can have  look at the xrt.ini file that is used for the current experiment it is present at the top level and copied to build folder that is used for host application launch. To open the run time profile summary report please follow the steps below:
