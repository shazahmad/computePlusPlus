...
Since we are simulating an image of 1000x30 so expected latency should be: 30,000 + fixed latency of some blocks(assuming one clock cycle per output pixel). The number shown in the report is 38,520. Here 8520 is the fixed latency and when the actual image size used is 1920x1080 it will get amortized on more number of image lines. The fact that a large image will amortize the latency can be verified by simulating with a larger image. Another thing that verifies that the kernel can achieve 1 output sample per cycle throughput is loop initiation intervals. The synthesis report expanded view shows this below( all loops have II=1):
