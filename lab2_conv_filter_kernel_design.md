...
This section discusses the design of a convolution filter in detail. It goes through top-level structure, optimizations, and implementation details.
...
vitis_hls -p conv_filter_prj
 ```
 After GUI opens the first thing to notice is the resource utilization report as shown below:
...
Since we are simulating an image of 1000x30 so expected latency should be: 30,000 + fixed latency of some blocks(assuming one clock cycle per output pixel). The number shown in the report is 38,520. Here 8520 is the fixed latency and when the image size used is actually 1920x1080 it will get amortized on more number of image lines. The fact that a large image will amortize the latency can be verified by simulating with a larger image. Another thing that verifies that the kernel can achieve 1 output sample per cycle throughput is loop initiation intervals. The synthesis report expanded view shows this below( all loops have II=1):
